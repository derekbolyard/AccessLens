...
{
    var groups = issues.GroupBy(i => new { i.Rule, i.IssueText, i.Html, i.Severity, i.Rank })
        .Select(g => new Summary(g.Key.Rule, g.Key.IssueText, g.Key.Html, g.Key.Severity,
            g.Select(x => x.Page).Distinct().Count(), g.First().Page, g.Key.Rank))
        .OrderBy(s => s.Rank).ThenByDescending(s => s.Pages)
        .ToList();
    return groups;
}
...
record Summary(string Rule, string Issue, string Html, string Severity, int Pages, string ExampleUrl, int Rank);
